...
const Category = () => {
  ...
  const fetchCategorys = useCallback(
    async (page: number, limit: number = 6) => {
      setLoading(true);
      try {
  ...
  const handleUnlist = async (categoryId: string) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: `Do you want to ${
        categorys.find((cat) => cat._id === categoryId)?.isListed
          ? "unlist"
          : "list"
      } this category?`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      ...
      if (response.data) {
        ...
        setCategorys((prevCategory) =>
          prevCategory.map((categorys) =>
            categorys._id === categoryId
              ? { ...categorys, isListed: !categorys.isListed }
              : categorys
          )
        );
      } else {
