...
export const signup = async(name: string, email: string, mobile: number, password: string) => {
    try {
        const response = await Api.post(userEndpoint.candidateSignUp, {name, email, mobile, password})
...
export const verifyOtp = async (otp: string) => {
    try {
        ...
        const response = await Api.post(userEndpoint.verifyOtp, {otp}, {
            headers: {
...
export const verifyLogin = async (email: string, password: string) => {
    try {
        const response = await Api.post(userEndpoint.verifyLogin, {email, password})
        return response;
...
export const editPassword = async (currentPassword: string, newPassword: string) => {
    try {
        const {data} = await Api.put(userEndpoint.editPassword, {currentPassword, newPassword})
        return data 
    } catch (error: any) {
        console.log(error)
...
export const getServiceProviderDetails = async (id: string) => {
    try {
        const response = await Api.get(userEndpoint.getServiceProviderDetails+`/${id}`)
...
export const getServiceProviderSlotDetails = async (serviceProviderId: string) => {
    try {
        const response = await Api.get(userEndpoint.getInterviewerSlotDetails + `/${serviceProviderId}`, {
...
export const makePayment = async (data: any, previousUrl: string)=> {
    try {
        const response = await Api.post(userEndpoint.makePayment, {data, previousUrl})
        return response.data
    } catch (error) {
        console.log(error)
    }
...
export const getScheduledIbookings = async (page: number, limit: number) => {
    try {
        const response = await Api.get(userEndpoint.getScheduledBookings+`?page=${page}&limit=${limit}`)
        return response.data
    } catch (error) {
        console.log(error)
    }
...
export const fetchBlogs = async (page: number, limit: number) => {
    try {
        const response = await Api.get(`${userEndpoint.getBlogs}?page=${page}&limit=${limit}`);
        return response.data; // Assumes the API response has data in the body
    } catch (error) {
  ...
  export const cancelBooking = async (bookingId: string, cancellationReason: string) => {
    try {
        const response = await Api.post(`${userEndpoint.cancelBooking}/${bookingId}`, { cancellationReason });
